...
function load_params(path::AbstractString)
    ...
    model_config = ScoreUNetConfig(
        in_channels=Int(get(model_cfg, "in_channels", JOINT_INPUT_CHANNELS)),
    ...
    require_condition(params.model_config.in_channels == JOINT_INPUT_CHANNELS, "The L96 joint-score U-Net expects in_channels = 3.")
    require_condition(params.model_config.periodic, "L96 joint score training should use periodic convolutions.")
...
function apply_pair_stats(samples::Array{Float32, 3}, stats::DataStats)
    mean_tensor = reshape(permutedims(stats.mean, (2, 1)), size(samples, 1), 1, 1)
    std_tensor = reshape(permutedims(stats.std, (2, 1)), size(samples, 1), 1, 1)
...
function compute_state_stats(sampler::JointPairSampler, shared_normalization::Bool)
    post_states = sampler.states[sampler.start_idx:end, :, :]
    flat = reshape(permutedims(post_states, (2, 1, 3)), size(sampler.states, 2), 1, :)
    return shared_normalization ? shared_data_stats(flat) : sitewise_data_stats(flat)
end
...
function sample_pair_batch!(state_buffer::Array{Float32, 3}, tnorm_buffer::Vector{Float32},
    ...
    Threads.@threads for sample_idx in 1:batch_size
        ...
        @inbounds for mode_idx in 1:K
            state_buffer[mode_idx, 1, sample_idx] = sampler.states[time_idx, mode_idx, traj_idx]
            state_buffer[mode_idx, 2, sample_idx] = sampler.states[time_idx + lag, mode_idx, traj_idx]
        end
...
function random_pair_tensor(sampler::JointPairSampler, lag::Int, tnorm::Float32, npairs::Int, rng::AbstractRNG)
    ...
    @inbounds for sample_idx in 1:npairs
        ...
        for mode_idx in 1:K
            pair_tensor[mode_idx, 1, sample_idx] = sampler.states[time_idx, mode_idx, traj_idx]
            pair_tensor[mode_idx, 2, sample_idx] = sampler.states[time_idx + lag, mode_idx, traj_idx]
        end
    end
...
function encode_joint_input!(input_buffer::Array{Float32, 3}, normalized_pair::Array{Float32, 3},
    ...
    @inbounds for sample_idx in 1:batch
        ...
        for mode_idx in 1:K
            ...
            input_buffer[mode_idx, 3, sample_idx] = tval
        end
    end
    return nothing
end
...
function train_model(sampler::JointPairSampler, stats::DataStats, params::L96JointScoreParams,
    ...
    model = JointScoreUNet(build_unet(model_cfg))
    Random.seed!()
    ...
    input_cpu = Array{Float32}(undef, K, JOINT_INPUT_CHANNELS, B)
    tnorm_cpu = Vector{Float32}(undef, B)
    noise_cpu = Array{Float32}(undef, K, JOINT_STATE_CHANNELS, B)

    noisy_device = device isa GPUDevice ? CUDA.CuArray{Float32}(undef, K, JOINT_INPUT_CHANNELS, B) : Array{Float32}(undef, K, JOINT_INPUT_CHANNELS, B)
    ...
    for epoch in 1:params.trainer_config.epochs
        ...
        for _ in 1:params.batches_per_epoch
            ...
            loss_value, grads = Flux.withgradient(model) do current_model
                ...
                Flux.Losses.mse(pred, noise_device) / params.trainer_config.accumulation_steps
            end
...
function integrate_joint_score_sde(model, init_states::Array{Float32, 3}, tnorm::Float32,
    ...
    t_dev = move_array(fill(Float32(tnorm), K, 1, nchains), device)
    input_dev = move_array(zeros(Float32, K, JOINT_INPUT_CHANNELS, nchains), device)
    ...
    progress = cfg.progress ? Progress(cfg.nsteps; desc="L96 joint score Langevin", dt=0.5) : nothing

    for step in 1:cfg.nsteps
        @views input_dev[:, 1:JOINT_STATE_CHANNELS, :] .= state_dev
        @views input_dev[:, 3:3, :] .= t_dev
...
function draw_offset_pair_samples(samples::Array{Float32, 3}, offset::Int, max_samples::Int, rng::AbstractRNG)
    ...
    if max_samples <= 0 || max_samples >= total
        ...
        @inbounds for snap_idx in 1:nsnaps
            for mode_idx in 1:K
                ...
                y_values[cursor] = samples[paired_idx, 2, snap_idx]
                cursor += 1
            end
        end
    ...
    for sample_idx in 1:max_samples
        ...
        y_values[sample_idx] = samples[paired_idx, 2, snap_idx]
    end
    return x_values, y_values
end
...
function stein_matrix(model, normalized_pairs::Array{Float32, 3}, tnorm::Float32, sigma::Float32,
    ...
    noisy_input = Array{Float32}(undef, size(clean_batch, 1), JOINT_INPUT_CHANNELS, size(clean_batch, 3))
...
function evaluate_tau(model, sampler::JointPairSampler, stats::DataStats, lag::Int, tau::Float64, tnorm::Float32,
    ...
    xt_centers, observed_xt_density = marginal_density_from_samples(observed_raw, 2, params.pdf_bins,
    ...
    _, generated_xt_density = marginal_density_from_samples(generated_raw, 2, params.pdf_bins,
    ...
    stein_mat = stein_matrix(model, observed_norm, tnorm, params.trainer_config.sigma, params.stein_samples, stein_rng, device)
...
function marginal_panel!(parent, record::L96JointEvalRecord)
    ax = Axis(parent; xlabel="value", ylabel="density",
        title=@sprintf("Marginal densities  tau=%.2f", record.tau))
    lines!(ax, record.x0_centers, record.observed_x0_density; color=STYLE_PRIMARY, label="x0 data")
    lines!(ax, record.x0_centers, record.generated_x0_density; color=STYLE_PRIMARY, linestyle=:dash, label="x0 gen")
    lines!(ax, record.xt_centers, record.observed_xt_density; color=STYLE_ACCENT, label="xt data")
    lines!(ax, record.xt_centers, record.generated_xt_density; color=STYLE_ACCENT, linestyle=:dash, label="xt gen")
...
function contour_pair_panel!(parent, observed::PairPdfResult, generated::PairPdfResult, tau::Float64, kl::Float64)
    ...
    ax = Axis(parent; xlabel="x0_i", ylabel=@sprintf("x_tau,i+%d", observed.offset),
...
function create_diagnostics_figure(output_path::AbstractString, params::L96JointScoreParams,
    ...
    with_scaled_figure_style(params.figure_width, params.figure_height) do _
        ...
        figure_title!(fig, "L96 lag-conditioned joint score diagnostics"; subtitle=subtitle)
