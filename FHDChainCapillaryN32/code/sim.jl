...
# Simulates the N=16 capillary strong-state-dependence periodic fluctuating-
# hydrodynamic chain. This script deliberately stops at data generation:
...
struct SimParams
    ...
    zeta::Float64
    kappa::Float64
    density_correlation_length_dx::Float64
    t0::Float64
    dt::Float64
    ntrajectories::Int
    ...
    output_structure_factor_png::String
end
...
struct StructureFactorResult
    ...
    laplacian_symbol::Vector{Float64}
    empirical::Vector{Float64}
    theory_shape::Vector{Float64}
    fitted_theory::Vector{Float64}
    fitted_amplitude::Float64
    rel_rmse::Float64
    corr::Float64
end
...
function load_params(path::AbstractString)
    ...
    N_cfg = Int(sim["N"])
    L_cfg = Float64(sim["L"])
    rho0_cfg = Float64(sim["rho0"])
    cs_cfg = Float64(sim["sound_speed"])
    xi_dx = Float64(get(sim, "density_correlation_length_dx", NaN))
    kappa_cfg = isfinite(xi_dx) && xi_dx > 0.0 ?
        (xi_dx * L_cfg / N_cfg)^2 * cs_cfg^2 / rho0_cfg :
        Float64(get(sim, "kappa", 2.0e-2))
    ...
    params = SimParams(
        ...
        String(get(out, "structure_factor_figure_png", "../figures/static_structure_factor.png")),
    )
    ...
    require_condition(params.kappa > 0.0, "kappa must be positive.")
    require_condition(!isfinite(params.density_correlation_length_dx) ||
        params.density_correlation_length_dx > 0.0,
        "density_correlation_length_dx must be positive when specified.")
...
function edge_viscosity!(eta::Vector{Float64}, z::AbstractMatrix{<:Real}, params::SimParams)
    N = params.N
...
function density_laplacian!(laprho::Vector{Float64}, z::AbstractMatrix{<:Real}, params::SimParams)
    N = params.N
    h = dx(params)
    inv_h2 = inv(h * h)
    @inbounds for i in 1:N
        ...
        ip1 = periodic_index(i + 1, N)
        laprho[i] = (z[ip1, 1] - 2.0 * z[i, 1] + z[im1, 1]) * inv_h2
    end
    return nothing
end

function capillary_flux!(pi_cap::Vector{Float64}, laprho::Vector{Float64},
                         z::AbstractMatrix{<:Real}, params::SimParams)
    N = params.N
    h = dx(params)
    kappa = params.kappa
    @inbounds for i in 1:N
        ip1 = periodic_index(i + 1, N)
        rho_bar = 0.5 * (z[i, 1] + z[ip1, 1])
        grad_rho = (z[ip1, 1] - z[i, 1]) / h
        lap_edge = 0.5 * (laprho[i] + laprho[ip1])
        pi_cap[i] = -kappa * rho_bar * lap_edge + 0.5 * kappa * grad_rho^2
    end
    return nothing
end

function capillary_force!(force::Vector{Float64}, pi_cap::Vector{Float64}, params::SimParams)
    N = params.N
    h = dx(params)
    @inbounds for i in 1:N
        im1 = periodic_index(i - 1, N)
        force[i] = -(pi_cap[i] - pi_cap[im1]) / h
    end
    return nothing
end

function analytic_score!(score::Array{Float64, 2}, laprho::Vector{Float64},
                         z::AbstractMatrix{<:Real}, params::SimParams)
    N = params.N
    h = dx(params)
    inv_theta = inv(params.theta)
    density_mean = 0.0
    momentum_mean = 0.0
    @inbounds for i in 1:N
        rho_i = z[i, 1]
        rho_i > 0.0 || error(@sprintf("Nonpositive density %.6e at site %d.", rho_i, i))
        u_i = z[i, 2] / rho_i
        score[i, 1] = -h * inv_theta *
            (params.cs^2 * log(rho_i / params.rho0) - 0.5 * u_i^2 - params.kappa * laprho[i])
        score[i, 2] = -h * inv_theta * u_i
        density_mean += score[i, 1]
        momentum_mean += score[i, 2]
    end
    density_mean /= N
    momentum_mean /= N
    @inbounds for i in 1:N
        score[i, 1] -= density_mean
        score[i, 2] -= momentum_mean
    end
    return nothing
end

function free_energy_kappa(z::AbstractMatrix{<:Real}, params::SimParams)
    N = params.N
    h = dx(params)
    local_energy = 0.0
    gradient_energy = 0.0
    @inbounds for i in 1:N
        ...
        rho_i > 0.0 || error(@sprintf("Nonpositive density %.6e at site %d.", rho_i, i))
        m_i = z[i, 2]
        local_energy += params.cs^2 * (rho_i * log(rho_i / params.rho0) - rho_i + params.rho0) +
            0.5 * m_i^2 / rho_i
        gradient_energy += (z[ip1, 1] - rho_i)^2
    end
    return h * local_energy + 0.5 * params.kappa * gradient_energy / h
end

function capillary_verification(z::AbstractMatrix{<:Real}, params::SimParams; seed::Int=1234)
    N = params.N
    laprho = Vector{Float64}(undef, N)
    pi_cap = Vector{Float64}(undef, N)
    force = Vector{Float64}(undef, N)
    score = Array{Float64}(undef, N, 2)
    density_laplacian!(laprho, z, params)
    capillary_flux!(pi_cap, laprho, z, params)
    capillary_force!(force, pi_cap, params)
    analytic_score!(score, laprho, z, params)

    h = dx(params)
    capillary_term_error = 0.0
    @inbounds for i in 1:N
        rho_i = z[i, 1]
        u_i = z[i, 2] / rho_i
        density_unc = -h / params.theta *
            (params.cs^2 * log(rho_i / params.rho0) - 0.5 * u_i^2 - params.kappa * laprho[i])
        density_no_kappa_unc = -h / params.theta *
            (params.cs^2 * log(rho_i / params.rho0) - 0.5 * u_i^2)
        capillary_term_error = max(capillary_term_error,
            abs((density_unc - density_no_kappa_unc) - h * params.kappa * laprho[i] / params.theta))
    end

    rng = MersenneTwister(seed)
    direction = randn(rng, N, 2)
    direction[:, 1] .-= mean(@view direction[:, 1])
    direction[:, 2] .-= mean(@view direction[:, 2])
    direction ./= max(norm(direction), eps(Float64))
    max_density_step = minimum(@view z[:, 1]) / max(maximum(abs, @view direction[:, 1]), eps(Float64))
    eps_fd = min(1.0e-5, 0.1 * max_density_step)
    z_plus = z .+ eps_fd .* direction
    z_minus = z .- eps_fd .* direction
    fd_log_density_direction =
        (-(free_energy_kappa(z_plus, params) - free_energy_kappa(z_minus, params)) / params.theta) /
        (2.0 * eps_fd)
    score_direction = sum(score .* direction)
    fd_score_abs_error = abs(fd_log_density_direction - score_direction)
    fd_score_rel_error = fd_score_abs_error / max(abs(fd_log_density_direction), abs(score_direction), eps(Float64))

    return (
        capillary_force_sum = sum(force),
        capillary_force_max_abs = maximum(abs, force),
        capillary_score_term_max_abs_error = capillary_term_error,
        constrained_score_fd_abs_error = fd_score_abs_error,
        constrained_score_fd_rel_error = fd_score_rel_error,
        free_energy = free_energy_kappa(z, params),
    )
end

function drift!(b::Array{Float64, 2}, eta::Vector{Float64}, laprho::Vector{Float64},
                pi_cap::Vector{Float64}, z::Array{Float64, 2}, params::SimParams)
    N = params.N
    h = dx(params)
    ...
    @inbounds for i in 1:N
        ...
        b[i, 2] = -((pi_right + pi_cap[i]) - (pi_left + pi_cap[im1])) / h +
            (tau_right - tau_left) / h
    end
    return nothing
end
...
function noise_increment!(dm_noise::Vector{Float64}, eta::Vector{Float64}, rng::AbstractRNG,
    ...
    @inbounds for edge in 1:N
        ...
        amp = sqrt(2.0 * params.theta * eta[edge] / h) / h * sqrt(dt) * randn(rng)
        dm_noise[edge] += amp
        dm_noise[ip1] -= amp
    end
    return nothing
...
function euler_candidate!(candidate::Array{Float64, 2}, z::Array{Float64, 2},
                          b::Array{Float64, 2}, eta::Vector{Float64},
                          laprho::Vector{Float64}, pi_cap::Vector{Float64},
                          dm_noise::Vector{Float64},
                          rng::AbstractRNG, params::SimParams, dt::Float64)
    drift!(b, eta, laprho, pi_cap, z, params)
...
function positive_step!(z::Array{Float64, 2}, candidate::Array{Float64, 2},
    ...
    if euler_candidate!(candidate, z, b, eta, laprho, pi_cap, dm_noise, rng, params, dt)
        copyto!(z, candidate)
        return 0
    end
    ...
    retries = 1
    retries += positive_step!(z_half, candidate_half, b, eta, laprho, pi_cap, dm_noise,
        rng, params, 0.5dt, depth + 1)
    retries += positive_step!(z_half, candidate_half, b, eta, laprho, pi_cap, dm_noise,
        rng, params, 0.5dt, depth + 1)
    copyto!(z, z_half)
    return retries
...
function integrate_fhd_ensemble(params::SimParams, t1::Float64, save_dt::Float64; label::AbstractString, dt::Float64=params.dt)
    ...
    require_condition(isapprox(nsteps_float, round(nsteps_float); atol=1e-6), "t1 - t0 must be an integer multiple of dt.")
    ...
    states = Array{Float32}(undef, nsaved, params.N, 2, params.ntrajectories)
    ...
    Threads.@threads :dynamic for traj_idx in 1:params.ntrajectories
        ...
        @inbounds for step in 1:nsteps
            substep_retries += positive_step!(z, candidate, b, eta, laprho, pi_cap,
                dm_noise, rng, params, dt)
            if traj_idx == 1 && step % heartbeat_stride == 0
                @printf("%s trajectory 1 progress %.1f%% at step %d / %d\n",
                    label, 100.0 * step / nsteps, step, nsteps)
                flush(stdout)
            end
...
function density_structure_factor(states::AbstractArray{<:Real, 4}, start_idx::Int,
                                  params::SimParams)
    nt, N, _, ntraj = size(states)
    max_mode = iseven(N) ? N ÷ 2 - 1 : N ÷ 2
    modes = collect(1:max_mode)
    h = dx(params)
    wavenumbers = 2pi .* modes ./ params.L
    lap_symbol = 4.0 .* sin.(pi .* modes ./ N).^2 ./ h^2
    empirical = zeros(Float64, length(modes))
    scratch = Vector{ComplexF64}(undef, N)
    count = 0
    @inbounds for traj in 1:ntraj, t in start_idx:nt
        rho_mean = mean(@view states[t, :, 1, traj])
        for i in 1:N
            scratch[i] = ComplexF64(Float64(states[t, i, 1, traj]) - rho_mean, 0.0)
        end
        rhohat = fft(scratch)
        for (mi, mode) in enumerate(modes)
            empirical[mi] += abs2(rhohat[mode + 1]) / N
        end
        count += 1
    end
    empirical ./= max(count, 1)
    theory_shape = 1.0 ./ (params.cs^2 / params.rho0 .+ params.kappa .* lap_symbol)
    amp = dot(empirical, theory_shape) / max(dot(theory_shape, theory_shape), eps(Float64))
    fitted = amp .* theory_shape
    rel = norm(empirical .- fitted) / max(norm(empirical), eps(Float64))
    cc = length(empirical) > 1 ? cor(empirical, fitted) : 1.0
    return StructureFactorResult(modes, wavenumbers, lap_symbol, empirical,
        theory_shape, fitted, amp, rel, cc)
end

function render_structure_factor_figure(path::AbstractString, params::SimParams,
                                        ssf::StructureFactorResult)
    fig = Figure(; size=(1800, 1300))
    xi = sqrt(params.kappa * params.rho0 / params.cs^2)
    subtitle = @sprintf("kappa=%.6g from xi/dx=%.3f; fitted S_rho shape rel.RMSE=%.3e corr=%.4f",
        params.kappa, xi / dx(params), ssf.rel_rmse, ssf.corr)
    figure_title!(fig, "Density static structure factor validation"; subtitle=subtitle)

    ax = Axis(fig[1, 1]; title="Capillary density spectrum",
        xlabel="discrete mode n", ylabel="S_rho(n), fitted scale")
    scatter!(ax, ssf.modes, ssf.empirical; color=STYLE_PRIMARY, markersize=13, label="simulation")
    lines!(ax, ssf.modes, ssf.fitted_theory; color=STYLE_SECONDARY, linewidth=4,
        linestyle=:dash, label="A / (c_s^2/rho0 + kappa lambda_n)")
    axislegend(ax; position=:rt)

    ax_norm = Axis(fig[1, 2]; title="Normalized shape",
        xlabel="discrete mode n", ylabel="S_rho(n) / S_rho(1)")
    lines!(ax_norm, ssf.modes, ssf.empirical ./ ssf.empirical[1];
        color=STYLE_PRIMARY, linewidth=4, label="simulation")
    lines!(ax_norm, ssf.modes, ssf.fitted_theory ./ ssf.fitted_theory[1];
        color=STYLE_SECONDARY, linewidth=4, linestyle=:dash, label="capillary prediction")
    axislegend(ax_norm; position=:rt)

    ax_res = Axis(fig[2, 1]; title="Relative residual by mode",
        xlabel="discrete mode n", ylabel="(S-fit)/S")
    hlines!(ax_res, [0.0]; color=STYLE_ZERO, linestyle=:dash)
    lines!(ax_res, ssf.modes, (ssf.empirical .- ssf.fitted_theory) ./ ssf.empirical;
        color=STYLE_HIGHLIGHT, linewidth=3)

    ax_meta = Axis(fig[2, 2]; title="Physical parameterization")
    hidedecorations!(ax_meta)
    hidespines!(ax_meta)
    lines_txt = [
        @sprintf("dx = %.8f", dx(params)),
        @sprintf("target xi = %.8f", xi),
        @sprintf("target xi/dx = %.4f", xi / dx(params)),
        @sprintf("kappa = (xi^2 c_s^2/rho0) = %.8f", params.kappa),
        @sprintf("rho stiffness: c_s^2/rho0 = %.8f", params.cs^2 / params.rho0),
        "prediction uses discrete lambda_n = 4 sin^2(pi n/N)/dx^2",
        "zero mode excluded because total mass is conserved",
    ]
...
function channel_mean_var_nonzero(states::AbstractArray{<:Real, 4}, start_idx::Int, channel::Int)
    nt, N, _, ntraj = size(states)
    total = 0.0
    count = 0
    @inbounds for traj in 1:ntraj, t in start_idx:nt
...
function downsample_states(states::AbstractArray{<:Real, 4}, start_idx::Int, stride::Int)
    indices = collect(start_idx:stride:size(states, 1))
    return Array(@view states[indices, :, :, :])
end

function compute_fhd_correlations(data::AbstractArray{<:Real, 4}, dt_corr::Float64, threshold::Float64,
...
function draw_scalar_samples(states::AbstractArray{<:Real, 4}, start_idx::Int, max_samples::Int,
...
function compute_marginal_pdfs(states::AbstractArray{<:Real, 4}, start_idx::Int, params::SimParams)
...
function draw_pair_samples(states::AbstractArray{<:Real, 4}, start_idx::Int, max_samples::Int,
                           rng::AbstractRNG, selector::Function)
    nt, N, _, ntraj = size(states)
    npost = nt - start_idx + 1
    total = npost * N * ntraj
...
function compute_pair_pdfs(states::AbstractArray{<:Real, 4}, start_idx::Int, offsets::Vector{Int},
                           params::SimParams)
...
function compute_spatial_covariance(states::AbstractArray{<:Real, 4}, start_idx::Int)
    nt, N, _, ntraj = size(states)
    D = 2N
...
function viscosity_summary(states::AbstractArray{<:Real, 4}, start_idx::Int, params::SimParams)
    nt, N, _, ntraj = size(states)
    eta_total = 0.0
    eta2_total = 0.0
    rho_min = Inf
...
function compute_conservation_stats(times::Vector{Float64}, states::AbstractArray{<:Real, 4}, params::SimParams)
    nt, N, _, ntraj = size(states)
    h = dx(params)
...
function save_hdf5(path::AbstractString, params::SimParams, ref::PreviousReference, schedule::ProductionSchedule,
                   pilot_corr::CorrelationResult, times::Vector{Float64}, states::AbstractArray{<:Real, 4},
                   ...
                   conservation::ConservationStats, ssf::StructureFactorResult)
    nt, N, _, ntraj = size(states)

    h5open(path, "w") do file
        file["/trajectories/time"] = times
        file["/trajectories/states"] = states
        states_flat = create_dataset(file, "/trajectories/states_flat", Float32, (nt, 2N, ntraj))
        @inbounds for traj in 1:ntraj
            states_flat[:, 1:N, traj] = @view states[:, :, 1, traj]
            states_flat[:, (N + 1):(2N), traj] = @view states[:, :, 2, traj]
        end
        ...
        file["/statistics/static_structure_factor/modes"] = ssf.modes
        file["/statistics/static_structure_factor/wavenumbers"] = ssf.wavenumbers
        file["/statistics/static_structure_factor/discrete_laplacian_symbol"] = ssf.laplacian_symbol
        file["/statistics/static_structure_factor/empirical"] = ssf.empirical
        file["/statistics/static_structure_factor/theory_shape"] = ssf.theory_shape
        file["/statistics/static_structure_factor/fitted_theory"] = ssf.fitted_theory
        file["/statistics/static_structure_factor/fitted_amplitude"] = ssf.fitted_amplitude
        file["/statistics/static_structure_factor/shape_rel_rmse"] = ssf.rel_rmse
        file["/statistics/static_structure_factor/shape_correlation"] = ssf.corr
        ...
        capillary_check = capillary_verification(Array(states[end, :, :, 1]), params; seed=params.seed + 17)
        file["/statistics/capillary/capillary_force_sum"] = capillary_check.capillary_force_sum
        file["/statistics/capillary/capillary_force_max_abs"] = capillary_check.capillary_force_max_abs
        file["/statistics/capillary/score_kappa_term_max_abs_error"] =
            capillary_check.capillary_score_term_max_abs_error
        file["/statistics/capillary/constrained_score_finite_difference_abs_error"] =
            capillary_check.constrained_score_fd_abs_error
        file["/statistics/capillary/constrained_score_finite_difference_rel_error"] =
            capillary_check.constrained_score_fd_rel_error
        file["/statistics/capillary/free_energy_sample"] = capillary_check.free_energy

        file["/metadata/model_name"] = "periodic_fluctuating_hydrodynamic_chain_capillary_strong_state_dependent_mobility"
        ...
        if isfinite(params.density_correlation_length_dx)
            file["/metadata/density_correlation_length_dx"] = params.density_correlation_length_dx
            file["/metadata/kappa_selection"] = "kappa = (density_correlation_length_dx*dx)^2*c_s^2/rho0"
        else
            file["/metadata/density_correlation_length_dx"] = sqrt(params.kappa * params.rho0 / params.cs^2) / dx(params)
            file["/metadata/kappa_selection"] = "kappa specified directly; implied xi/dx stored"
        end
        file["/metadata/regime"] = "capillary_strong_state_dependence"
        file["/metadata/free_energy"] = "F_kappa = dx*sum_i[c_s^2*(rho_i*log(rho_i/rho0)-rho_i+rho0)+m_i^2/(2rho_i)] + kappa/(2dx)*sum_i(rho_{i+1}-rho_i)^2"
        file["/metadata/analytic_score"] = "s_rho_i = -dx/Theta*(c_s^2*log(rho_i/rho0)-0.5*u_i^2-kappa*Delta_h rho_i), s_m_i = -dx/Theta*u_i, then subtract channel means"
        ...
        file["/metadata/sde_noise_prefactor"] = "conservative edge stress sqrt(2*Theta*eta_edge/dx)/dx"
        ...
        file["/metadata/momentum_update"] = "Euler-Maruyama conservative flux/noise update with Korteweg capillary reversible flux, velocity denominator floor, and zero-mode projection"
    end
    return nothing
end
...
function render_summary_figure(path::AbstractString, params::SimParams, ref::PreviousReference,
                               ...
                               pair_pdfs::Vector{PairPdf}, corr::CorrelationResult, cov::Matrix{Float64},
                               mean_eta::Float64, std_eta::Float64, rho_min::Float64, rho_max::Float64,
                               conservation::ConservationStats, sample_state::AbstractMatrix{<:Real})
    ...
    figure_title!(fig, "Capillary fluctuating-hydrodynamic chain - observational summary"; subtitle=subtitle)

    rho_rel_centers = (rho_pdf.centers .- params.rho0) ./ params.rho0
    rho_rel_density = rho_pdf.density .* params.rho0
    mach_centers = u_pdf.centers ./ params.cs
    mach_density = u_pdf.density .* params.cs
    ax_pdf = Axis(fig[1, 1]; title="Nondimensional cell marginals", xlabel="value", ylabel="density")
    lines!(ax_pdf, rho_rel_centers, rho_rel_density; color=STYLE_PRIMARY, linewidth=3,
        label="(rho-rho0)/rho0")
    lines!(ax_pdf, mach_centers, mach_density; color=STYLE_SECONDARY, linewidth=3,
        label="u/c_s")
    axislegend(ax_pdf; position=:rt)

    ax_u = Axis(fig[1, 2]; title="Momentum and edge-viscosity PDFs", xlabel="value", ylabel="density")
    lines!(ax_u, m_pdf.centers, m_pdf.density; color=STYLE_HIGHLIGHT, linewidth=3, label="m")
...
function edge_viscosity_matrix(states::AbstractArray{<:Real, 4}, indices::Vector{Int}, traj::Int, params::SimParams)
...
function render_dynamics_figure(path::AbstractString, params::SimParams, schedule::ProductionSchedule,
                                times::Vector{Float64}, states::AbstractArray{<:Real, 4},
    ...
    figure_title!(fig, "Capillary strong FHD chain dynamics"; subtitle=subtitle)
...
function render_trajectories_figure(path::AbstractString, params::SimParams, schedule::ProductionSchedule,
    ...
    figure_title!(fig, "Capillary strong FHD chain selected trajectories"; subtitle=subtitle)
...
function run_pipeline(param_file::AbstractString)
    ...
    output_structure_factor_png = resolve_path(base_dir, params.output_structure_factor_png)
    ...
    ensure_parent_dir(output_structure_factor_png)
    ...
    ssf = density_structure_factor(states, start_idx, params)
    ...
    @printf("Density structure factor: shape rel.RMSE=%.6e, corr=%.6f, xi/dx=%.3f, kappa=%.6g\n",
        ssf.rel_rmse, ssf.corr, sqrt(params.kappa * params.rho0 / params.cs^2) / dx(params), params.kappa)
    ...
    save_hdf5(output_hdf5, params, ref, schedule, pilot_corr, times, states,
        ...
        rho_min, rho_max, conservation, ssf)

    sample_state = Array(states[end, :, :, 1])
    capillary_check = capillary_verification(sample_state, params; seed=params.seed + 17)
    @printf("Capillary checks: sum force=%.6e, score kappa-term err=%.6e, constrained score FD rel.err=%.6e\n",
        capillary_check.capillary_force_sum,
        capillary_check.capillary_score_term_max_abs_error,
        capillary_check.constrained_score_fd_rel_error)
    ...
    @printf("Rendering static structure factor figure to %s\n", output_structure_factor_png)
    render_structure_factor_figure(output_structure_factor_png, params, ssf)

    @printf("Completed capillary FHD simulation. Pilot tD = %.6f, production T = %.6f, save_dt = %.6f\n",
