...
Base.@kwdef struct FHDScoreParams
    ...
    model_init_seed::Int
    model_init_from_bson::String
    model_init_output_scale::Float32
    output_parameterization::String
    local_hidden_width::Int
    local_hidden_layers::Int
    batch_size::Int
    epochs::Int
...
function load_params(path::AbstractString)
    ...
    params = FHDScoreParams(
        ...
        model_architecture=String(get(model, "architecture", "unet")),
        ...
        model_init_from_bson=String(get(model, "init_from_bson", "")),
        model_init_output_scale=Float32(get(model, "init_output_scale", 1.0)),
        output_parameterization=String(get(model, "output_parameterization", "noise")),
        local_hidden_width=Int(get(model, "local_hidden_width", 256)),
        local_hidden_layers=Int(get(model, "local_hidden_layers", 4)),
    ...
    require_condition(0.0f0 < params.sigma <= 0.05f0,
        "FHDChain score DSM sigma must be positive and no larger than 0.05.")
    ...
    require_condition(params.model_architecture in ("unet", "local_mlp", "local_feature_mlp"),
        "model.architecture must be one of: unet, local_mlp, local_feature_mlp.")
    require_condition(params.output_parameterization in ("noise", "score"),
        "model.output_parameterization must be noise or score.")
    return params
end
...
function score_regularization_terms(pred, noisy, sigma::Real, Ptarget, Pnorm2)
    K, C, B = size(pred)
    D = K * C
    inv_sigma = -one(eltype(pred)) / sigma
    score = pred .* inv_sigma
    # Julia's native flattening of (site, channel, batch) gives the documented
    # order (rho_1..rho_K, m_1..m_K) for each batch and stays GPU-friendly.
    sflat = reshape(score, D, B)
    xflat = reshape(noisy, D, B)
    invB = one(eltype(pred)) / B
    stein = -(sflat * transpose(xflat)) .* invB
    mean_s = sum(sflat; dims=2) .* invB
    stein_penalty = sum(abs2, stein .- Ptarget) / Pnorm2
    mean_penalty = sum(abs2, mean_s) / D
    return stein_penalty, mean_penalty
end

function score_prediction_from_training_output(pred, sigma::Real, parameterization::AbstractString)
    if parameterization == "score"
        return project_zero_modes(pred)
    end
    inv_sigma = -one(eltype(pred)) / sigma
    return project_zero_modes(pred .* inv_sigma)
end

function train_score_model(dataset::NormalizedDataset, params::FHDScoreParams,
        phys::FHDPhysicalParams, device::ExecutionDevice)
    if !isempty(strip(params.model_init_from_bson))
        init_path = resolve_path(@__DIR__, params.model_init_from_bson)
        require_condition(isfile(init_path), "model.init_from_bson not found: $(init_path)")
        blob = BSON.load(init_path)
        model = dict_get(blob, :host_model)
        model_cfg = haskey(blob, :model_cfg) ? blob[:model_cfg] : params.model_config
        if abs(params.model_init_output_scale - 1.0f0) > 1.0f-7
            model = OutputScaledModel(model, params.model_init_output_scale)
            @printf("Applied fixed output scale %.6g to initialized model\n",
                params.model_init_output_scale)
        end
        @printf("Initialized score model from %s\n", init_path)
    else
        Random.seed!(params.model_init_seed)
        model, model_cfg = if params.model_architecture == "local_mlp"
            build_local_score_mlp(params.local_hidden_width, params.local_hidden_layers,
                params.model_config.activation), params.model_config
        elseif params.model_architecture == "local_feature_mlp"
            build_local_feature_score_mlp(dataset.stats, phys, params.local_hidden_width,
                params.local_hidden_layers, params.model_config.activation), params.model_config
        else
            build_score_unet(params.model_config, params.model_normalization)
        end
        Random.seed!()
    end
    ...
    D = K * C
    Ptarget = move_array(Float32.(projection_matrix(K)), device)
    Pnorm2 = Float32(sum(abs2, to_host(Ptarget)))
    stein_weight = Float32(params.stein_weight)
    mean_score_weight = Float32(params.mean_score_weight)
    history = Dict(:train_loss => Float64[], :score_norm => Float64[],
        :param_norm => Float64[], :epoch_time => Float64[],
        :stein_penalty => Float64[], :mean_score_penalty => Float64[])
    ...
    for epoch in 1:cfg.epochs
        ...
        for part in Iterators.partition(idxs, cfg.batch_size)
            ...
            loss_value, grads = Flux.withgradient(model) do current_model
                ...
                dsm_loss = if params.output_parameterization == "score"
                    target_score = noise .* (-one(eltype(noise)) / cfg.sigma)
                    Flux.Losses.mse(pred, target_score)
                else
                    Flux.Losses.mse(pred, noise)
                end
                if stein_weight > 0.0f0 || mean_score_weight > 0.0f0
                    pred_score = score_prediction_from_training_output(pred, cfg.sigma,
                        params.output_parameterization)
                    stein_penalty, mean_penalty =
                        score_regularization_terms(-cfg.sigma .* pred_score, noisy,
                            cfg.sigma, Ptarget, Pnorm2)
                    dsm_loss + stein_weight * stein_penalty +
                        mean_score_weight * mean_penalty
                else
                    dsm_loss
                end
            end
        ...
        score = if params.output_parameterization == "score"
            project_zero_modes(model(monitor))
        else
            score_from_projected_model(model, monitor, cfg.sigma)
        end
        ...
        reg_n = min(4096, length(dataset))
        reg_noise_cpu = Array{Float32}(undef, K, C, reg_n)
        fill_projected_noise!(reg_noise_cpu, noise_rng)
        reg_clean = copy(dataset.data[:, :, 1:reg_n])
        reg_noisy = reg_clean .+ cfg.sigma .* reg_noise_cpu
        project_zero_modes!(reg_noisy)
        reg_pred = model(move_array(reg_noisy, device))
        reg_pred_noise = params.output_parameterization == "score" ?
            -cfg.sigma .* reg_pred : reg_pred
        stein_pen, mean_pen =
            score_regularization_terms(reg_pred_noise, move_array(reg_noisy, device),
                cfg.sigma, Ptarget, Pnorm2)
        push!(history[:stein_penalty], Float64(to_host(stein_pen)))
        push!(history[:mean_score_penalty], Float64(to_host(mean_pen)))
        progress !== nothing && ProgressMeter.next!(progress; showvalues=[
            (:epoch, epoch),
            (:loss, history[:train_loss][end]),
            (:score_norm, history[:score_norm][end]),
            (:stein_penalty, history[:stein_penalty][end]),
        ])
    end
...
function create_score_figure(path::AbstractString, params::FHDScoreParams,
    ...
    with_scaled_figure_style(params.figure_width, params.figure_height) do _
        ...
        lines = String[
            ...
            @sprintf("architecture = %s", params.model_architecture),
...
function save_score_model(path::AbstractString, model, model_cfg, dataset::NormalizedDataset,
    ...
    metadata = Dict(
        ...
        :training_output_parameterization => params.output_parameterization,
        :saved_output_parameterization => "noise_equivalent",
...
function run_pipeline(param_file::AbstractString)
    ...
    if params.train
        ...
        model, model_cfg, history = train_score_model(dataset, params, phys, device)
        eval_model = params.output_parameterization == "score" ?
            NoiseEquivalentScoreModel(model, params.sigma) : model
        checkpoint = string(output_bson, ".checkpoint")
        basic_diag = Dict{Symbol, Any}(:checkpoint => true)
        save_score_model(checkpoint, eval_model, model_cfg, dataset, params, phys, history, basic_diag,
            zeros(Float64, 2 * size(dataset.data, 1), 2 * size(dataset.data, 1)))
        if params.evaluate
            diagnostics, stein = score_diagnostics(eval_model, dataset, phys, params, device)
            scatter_exact, scatter_pred = diagnostic_scatter_data(eval_model, dataset, phys, params, device)
            save_score_model(output_bson, eval_model, model_cfg, dataset, params, phys, history, diagnostics, stein)
            ...
            save_score_model(output_bson, eval_model, model_cfg, dataset, params, phys, history, basic_diag,
                zeros(Float64, 2 * size(dataset.data, 1), 2 * size(dataset.data, 1)))
            rm(checkpoint; force=true)
